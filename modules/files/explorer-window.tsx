...
export const ExplorerWindow = observer(() => {
  ...
  return (
    <div className="flex min-h-0 grow flex-col bg-[#ede6da] text-[#26343d]">
      <div className="flex flex-row flex-wrap items-center gap-1 border-b border-[#26343d]/15 bg-[#f5f0e8] p-2">
        ...
        <div className="ml-1 flex min-w-0 grow flex-row items-center gap-1 overflow-auto rounded-md border border-[#26343d]/12 bg-white/60 px-2 py-1.5 text-xs">
          {workStore.path.split("/").filter(Boolean).map((part, index, parts) => {
            const path = `/${parts.slice(0, index + 1).join("/")}`
            return <button key={path} className="shrink-0 font-medium hover:text-[#9d3558]" onClick={() => navigate(path)}>{part}{index < parts.length - 1 && <span className="px-1 text-[#8a7773]">/</span>}</button>
          })}
        </div>
      ...
      <div className="flex min-h-0 grow flex-row">
        <aside className="hidden w-40 shrink-0 border-r border-[#26343d]/12 bg-[#e6dfd3] p-2 sm:block">
          <p className="px-2 pb-2 pt-1 font-mono text-[9px] uppercase tracking-[0.16em] text-[#8a7773]">Locations</p>
          <button className={cn("w-full rounded-md px-2 py-2 text-left text-sm", workStore.path.startsWith("/Work/Featured") ? "bg-white/70 font-medium" : "hover:bg-white/45")} onClick={() => navigate("/Work/Featured Work")}>Featured Work</button>
          <button className={cn("w-full rounded-md px-2 py-2 text-left text-sm", workStore.path.startsWith("/Work/Earlier") ? "bg-white/70 font-medium" : "hover:bg-white/45")} onClick={() => navigate("/Work/Earlier Work")}>Earlier Work</button>
        </aside>
      ...
      <div className="border-t border-[#26343d]/12 px-4 py-2 font-mono text-[10px] text-[#8a7773]">{items.length} {items.length === 1 ? "item" : "items"}</div>
    </div>
  )
...
function FileItem({ item, folder, selected, setRef, onKeyDown }: { item: FileSystemItem; folder: FolderFile; selected: boolean; setRef: (element: HTMLButtonElement | null) => void; onKeyDown: (event: KeyboardEvent, index: number) => void }) {
  ...
  return <button ref={setRef} role="option" aria-selected={selected} className={cn("group flex min-w-0 items-center gap-3 p-3 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[#9d3558]/60", selected ? "bg-[#9d3558]/12" : "hover:bg-white/55", workStore.view === "grid" ? "min-h-28 flex-col justify-start text-center" : "min-h-12 w-full flex-row")} onClick={() => workStore.select(item.id)} onPointerUp={(event) => { if (event.pointerType === "touch") openFile(item, folder) }} onDoubleClick={() => openFile(item, folder)} onKeyDown={(event) => onKeyDown(event, index)}>
...
function EmptyState({ message }: { message: string }) {
  return <div className="flex grow items-center justify-center p-8 text-center text-sm text-[#687279]"><p>{message}</p></div>
}
